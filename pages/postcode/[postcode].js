...
async function getPostCodeData(postcode) {
  let url = `https://data.nsw.gov.au/data/api/3/action/datastore_search_sql?sql=SELECT postcode, notification_date, confirmed_cases_count from "5d63b527-e2b8-4c42-ad6f-677f14433520" WHERE postcode='${postcode}'`;
  let response = await fetchRetry(url, 3);
  let json = await response.json();
  let data = json.result.records;
  let dataDates = data.map((item) => {
...
function getPostcodeDetails(postcode, data) {
...
export default function Home() {
  ...
  return (
    <>
      ...
      {postcodeDetails.all && postcodeDetails.week && postcodeDetails.day && (
        <Box
          sx={{
            paddingLeft: "1rem",
            paddingRight: "1rem",
        ...
        >
          <PostcodePageHeading
            ...
            suburbs={postcodeDetails.suburbs}
          />
          <PostcodePageNumbers
            all={postcodeDetails.all}
            week={postcodeDetails.week}
            day={postcodeDetails.day}
          />
        </Box>
      )}
